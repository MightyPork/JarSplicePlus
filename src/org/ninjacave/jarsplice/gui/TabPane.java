...
public class TabPane extends JPanel implements ActionListener {
	...
	public TabPane(JarSpliceFrame parent)
	{
		...
		final JPanel sideBarPanel = new JPanel(new BorderLayout());
		...
		final JPanel gapPanel = new JPanel(new BorderLayout());
	...
	private void addButton(JToggleButton button, boolean useTopButtonPanel)
	{
		...
		if (this.buttons.size() == 1) button.setSelected(true);
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		for (int i = 0; i < this.buttons.size(); i++) {
			final JToggleButton button = this.buttons.get(i);
